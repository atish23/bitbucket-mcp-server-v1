...
describe('BitbucketServer', () => {
    ...
    describe('Pull Request Operations', () => {
        const mockHandleRequest = async (toolName, args) => {
            const handlers = mockServer.setRequestHandler.mock.calls;
            const callHandler = handlers.find(([schema]) => schema.method === 'call_tool')?.[1];
            if (!callHandler)
            ...
            return callHandler(request, extra);
        };
        test('should create a pull request with explicit project', async () => {
            ...
            // Assert
            expect(mockAxios.post).toHaveBeenCalledWith('/projects/TEST/repos/repo/pull-requests', expect.objectContaining({
                title: input.title,
        ...
        test('should create a pull request using default project', async () => {
            ...
            const result = await mockHandleRequest('create_pull_request', input);
            // Assert
            expect(mockAxios.post).toHaveBeenCalledWith('/projects/DEFAULT/repos/repo/pull-requests', expect.objectContaining({
                title: input.title,
                description: input.description,
                fromRef: expect.any(Object),
                toRef: expect.any(Object),
        ...
        test('should throw error when no project is provided or defaulted', async () => {
            ...
            await expect(mockHandleRequest('create_pull_request', input))
                .rejects.toThrow(new McpError(ErrorCode.InvalidParams, 'Project must be provided either as a parameter or through BITBUCKET_DEFAULT_PROJECT environment variable'));
        });
        test('should merge a pull request', async () => {
            ...
            // Assert
            expect(mockAxios.post).toHaveBeenCalledWith('/projects/TEST/repos/repo/pull-requests/1/merge', expect.objectContaining({
                version: -1,
        ...
        test('should handle API errors', async () => {
            ...
            await expect(mockHandleRequest('get_pull_request', input))
                .rejects.toThrow(new McpError(ErrorCode.InternalError, 'Bitbucket API error: Not found'));
        });
    });
    describe('Reviews and Comments', () => {
        ...
        test('should filter review activities', async () => {
            ...
            expect(reviews.every((r) => ['APPROVED', 'REVIEWED'].includes(r.action))).toBe(true);
        });
        test('should add comment with parent', async () => {
            ...
            // Assert
            expect(mockAxios.post).toHaveBeenCalledWith('/projects/TEST/repos/repo/pull-requests/1/comments', {
                text: input.text,
